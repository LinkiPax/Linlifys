...
router.post('/Signin', async (req, res) => {
    ...
    try {
        ...
        // Check if user has completed personal details using your model's method
...
//     const { email, name, googleId, profilePicture, displayName, photos } = req.body;
...
//                 const baseUsername = (name || displayName || email.split('@')[0]).replace(/\s+/g, '').toLowerCase();
...
//                     profilePicture: profilePicture || (photos && photos[0]?.value),
//                     isVerified: true,
//                     isOnline: true,
...
router.get('/Signout', async (req, res) => {
    try {
        // If user ID is available in request, update online status
...
router.post('/Signup', [
    ...
    try {
        ...
        return res.status(201).json({
            ...
            user: user.toPublicJSON ? user.toPublicJSON() : user,
            token,
            redirectTo: `/personal-details/${user._id}` // Always redirect to personal details for new users
        });
    } catch (error) {
        console.error('Signup error:', error);
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        // Check if user can reset password (not a Google-only account)
        if (!user.canResetPassword || !user.canResetPassword()) {
            return res.status(400).json({ 
                message: 'This account uses Google authentication. Please sign in with Google.' 
            });
        }
...
router.get('/connections/:userId', async (req, res) => {
    try {
        const user = await User.findById(req.params.userId)
            .populate('connections', 'username profilePicture name jobTitle company isOnline lastSeen');
        
        if (!user) {
            return res.status(404).json({ error: 'User not found' });
        }
        
        res.json(user.connections.map(conn => 
            conn.toMinimalJSON ? conn.toMinimalJSON() : conn
        ));
    } catch (err) {
        console.error('Error fetching connections:', err);
        res.status(500).json({ error: 'Server error' });
...
router.get('/suggested/:userId', async (req, res) => {
    try {
        ...
        .select('username profilePicture name bio jobTitle company industry skills')
        .limit(10);
        
        res.json(suggestedUsers.map(u => 
            u.toMinimalJSON ? u.toMinimalJSON() : u
        ));
    } catch (err) {
        console.error('Error fetching suggested connections:', err);
        res.status(500).json({ error: 'Server error' });
...
router.patch('/add-connection/:userId', async (req, res) => {
    try {
        ...
        await User.findByIdAndUpdate(req.params.userId, {
            $addToSet: { connections: connectionId },
            $pull: { pendingRequests: connectionId, connectionRequests: connectionId }
        });
        
        // Add to other user's connections
        await User.findByIdAndUpdate(connectionId, {
            $addToSet: { connections: req.params.userId },
            $pull: { pendingRequests: req.params.userId, connectionRequests: req.params.userId }
        });
...
function getMissingFields(user) {
    ...
    return requiredFields.filter(fieldInfo => {
        const value = user[fieldInfo.field];
        return !value || value.toString().trim().length === 0;
    }).map(fieldInfo => fieldInfo.label);
}
