  ...
  return (
    <div className="status-viewer-container">
      ...
      <div className="status-viewer-bar">
        <div className="status-viewer-profile my-status-viewer" onClick={handleCreateClick}>
          <img
            className="status-viewer-pic"
            src={userProfilePic || "/default-profile.png"}
            alt="Your Status"
          />
          <div className="status-viewer-plus">
            <Plus size={16} />
          </div>
        </div>
        ...
        {groupedStatuses.map((userStatusGroup, index) => (
          <div
            key={userStatusGroup.user._id}
            className="status-viewer-profile"
            onClick={() => handleStatusClick(userStatusGroup, index)}
          >
            <img
              className="status-viewer-pic"
            ...
            />
            {userStatusGroup.statuses.length > 1 && (
              <div className="status-viewer-count">
                {userStatusGroup.statuses.length}
              </div>
            )}
          </div>
      ...
      {/* Status Modal */}
      {selectedStatus && (
        <div className="status-viewer-modal" onClick={closeModal}>
          <div className="status-viewer-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="status-progress-container">
              {groupedStatuses[currentUserIndex]?.statuses.map((_, idx) => (
                <div key={idx} className="status-progress-track">
                  <div
                    className={`status-progress-bar ${currentIndex === idx ? "status-progress-active" : ""}`}
                    style={{ 
                      width: currentIndex === idx ? `${progress}%` : 
                             idx < currentIndex ? "100%" : "0%" 
                    }}
            ...
            <div className="status-viewer-header">
              ...
              <div className="status-header-actions">
                {/* Music Indicator */}
                {selectedStatus.music && (
                  <div className="status-music-indicator">
                    <button 
                      className={`status-music-btn ${isMusicPlaying ? 'status-music-playing' : ''}`}
                      onClick={toggleMusic}
                      title={musicError || `Click to ${isMusicPlaying ? 'pause' : 'play'} music`}
                    >
                      <MusicNote size={16} />
                      <span>{formatMusicName(selectedStatus.music.name)}</span>
                      {musicError && <span className="status-music-error">⚠️</span>}
                    </button>
                  </div>
                )}
                ...
                {/* Delete button - only show if current user owns the status */}
                {selectedStatus.userId === userId && (
                  <div className="status-delete-menu" ref={deleteMenuRef}>
                    <button 
                    ...
                    {showDeleteMenu && (
                      <div className="status-delete-options">
                        <button 
                          className="status-delete-item"
                          onClick={() => handleDeleteStatus(selectedStatus._id)}
                        >
                          <Trash size={16} />
                          Delete Status
                ...
                <button className="status-close-btn" onClick={closeModal}>
                  <X size={20} />
                </button>
              </div>
            </div>
            ...
            <div className="status-viewer-actions">
              <div className="status-action-buttons">
                <button
                  className={`status-action-btn ${isLiked(selectedStatus) ? "status-action-liked" : ""}`}
                  onClick={() => handleLike(selectedStatus._id)}
                >
                  <Heart size={24} fill={isLiked(selectedStatus) ? "currentColor" : "none"} />
                  <span>{likeCount(selectedStatus)}</span>
                </button>
                <button
              ...
              {showComments && (
                <div className="status-comments-section">
                  <div className="status-comments-list">
                    {(comments[selectedStatus._id] || []).map((comment) => (
                      <div key={comment._id} className="status-comment">
                        <img
                          src={comment.userProfilePic || "/default-profile.png"}
                          alt={comment.userName}
                          className="status-comment-avatar"
                        />
                        <div className="status-comment-content">
                          <div className="status-comment-header">
                            <strong>{comment.userName}</strong>
                            {comment.userId === userId && (
                              <button
                                className="status-delete-comment"
                                onClick={() => handleDeleteComment(selectedStatus._id, comment._id)}
                              >
                                <X size={12} />
                              </button>
                  ...
                  {/* Comment Input */}
                  <div className="status-comment-input">
                    <input
                      type="text"
                      placeholder="Add a comment..."
                      value={commentText}
            ...
            {/* Navigation Buttons */}
            <button
              className="status-nav-btn status-prev-btn"
              onClick={goToPrevious}
              disabled={currentUserIndex === 0 && currentIndex === 0}
            >
              <ArrowLeft size={24} />
            ...
            <button
              className="status-nav-btn status-next-btn"
              onClick={goToNext}
            >
              <ArrowRight size={24} />
            </button>
