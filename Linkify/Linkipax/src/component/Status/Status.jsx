...
import { ArrowLeft, ArrowRight, X, Plus, ThreeDots, Heart, Chat, Send, Bookmark } from 'react-bootstrap-icons';
...
const Status = ({ userProfilePic }) => {
  ...
  const fetchStatuses = async () => {
    setLoading(true);
    try {
      const { data } = await axios.get('http://localhost:5000/api/statuses', {
        params: { userId }
      });
  ...
  const incrementViewCount = async (statusId) => {
    try {
      await axios.post(`http://localhost:5000/api/statuses/${statusId}/view`);
  ...
  const handleReaction = async (statusId) => {
    try {
      const { data } = await axios.post(`http://localhost:5000/api/statuses/${statusId}/like`, { userId });
      setReactions(prev => ({
        ...prev,
        [statusId]: {
          liked: data.liked,
    ...
    <div className="status-container">
      <div className="status-bar">
        ...
        {loading ? (
          ...
          statuses.map((status) => (
            <div key={status._id} className="status-profile" onClick={() => handleStatusClick(status)}>
              <img 
                className="profile-pic" 
        ...
        >
          <div className="status-content" onClick={(e) => e.stopPropagation()}>
              ...
              {selectedStatus.media.map((_, idx) => (
                <div key={idx} className="progress-track">
                  <div 
                    ...
                    style={{ width: currentIndex === idx ? `${progress}%` : idx < currentIndex ? '100%' : '0%' }}
                  ></div>
                </div>
              ))}
            </div>
            ...
            <div className="status-header">
              ...
              <button className="close-btn" onClick={closeModal} aria-label="Close"><X size={20} /></button>
            </div>

            <div className="media-container" onDoubleClick={() => handleReaction(selectedStatus._id)}>
              {selectedStatus.media[currentIndex].endsWith('.mp4') ? (
                <video 
                  ref={videoRef}
                  src={`http://localhost:5000/${selectedStatus.media[currentIndex]}`} 
            ...
            <div className="status-footer">
              <div className="action-buttons">
                <button 
                  className={`like-btn ${reactions[selectedStatus._id]?.liked ? 'active' : ''}`}
                  onClick={() => handleReaction(selectedStatus._id)}
                >
                  <Heart size={24} fill={reactions[selectedStatus._id]?.liked ? 'red' : 'none'} />
              ...
              {selectedStatus.caption && (
                <div className="status-caption">
                  <strong>{selectedStatus.user?.name || 'User'}</strong> {selectedStatus.caption}
                </div>
              )}
              ...
              <div className="status-meta">
                ...
                <span>{selectedStatus.createdAt ? new Date(selectedStatus.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'}) : ''}</span>
              </div>
            </div>

            <button className="nav-btn prev-btn" onClick={goToPrevious} disabled={currentIndex === 0} aria-label="Previous">
              <ArrowLeft size={24} />
            </button>
            <button className="nav-btn next-btn" onClick={goToNext} disabled={currentIndex === selectedStatus.media.length - 1} aria-label="Next">
              <ArrowRight size={24} />
            </button>
          </div>
        </div>
      ...
      {showPostModal && (
        <div className="modal-overlay" onClick={() => setShowPostModal(false)}>
          <div className="post-status-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3>Create Status</h3>
              <button className="close-btn" onClick={() => setShowPostModal(false)} aria-label="Close">
                <X size={20} />
              </button>
            </div>
              ...
              {mediaFiles.length > 0 ? (
                mediaFiles.map((file, index) => (
                  file.type.startsWith('video/') ? (
                    <video key={index} controls src={URL.createObjectURL(file)} className="preview-media" />
                  ) : (
                    <img key={index} src={URL.createObjectURL(file)} alt="preview" className="preview-media" />
                  )
                ))
              ) : (
            ...
            <div className="modal-actions">
              ...
              <button className="btn primary" onClick={handlePostStatus} disabled={mediaFiles.length === 0}>
                Post Status
              </button>
            </div>
          </div>
